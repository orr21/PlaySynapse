...
def transform(df_bronze: pl.DataFrame, *args, **kwargs) -> pl.DataFrame:
    ...
    df_final = df_final.with_columns(
        [
            ...
            pl.format("{} {}", pl.col("teamcity"), pl.col("teamname")).alias(
                "full_name"
            ),
            pl.format(
                "https://cdn.nba.com/logos/nba/{}/global/L/logo.svg", pl.col("teamid")
            ).alias("logo_url"),
    ...
    if "abbreviation" not in df_final.columns:
        if "teamslug" in df_final.columns:
            ...
            df_final = df_final.with_columns(
                pl.lit(None, dtype=pl.Utf8).alias("abbreviation")
            )
