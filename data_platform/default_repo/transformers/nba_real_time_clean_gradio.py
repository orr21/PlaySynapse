...
def fetch_team_names(game_id: str) -> Optional[Dict[str, str]]:
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            info = {
                "home_full": f"{h.get('teamCity')} {h.get('teamName')}".strip()
                or "Local",
                "away_full": f"{a.get('teamCity')} {a.get('teamName')}".strip()
                or "Visitante",
...
def generar_prompt_nba(row: Dict[str, Any]) -> str:
    ...
    if tipo in ["2pt", "3pt", "shot"]:
        ...
        if resultado == "MADE":
            ...
            ficha_dinamica.append(
                f"- PUNTOS TOTALES JUGADOR: {row.get('pointsTotal', 'N/A')}"
            )
            if row.get("shotDistance"):
                ficha_dinamica.append(f"- DISTANCIA: {row.get('shotDistance')} pies")
            if row.get("assistPlayerNameInitial"):
                ficha_dinamica.append(
                    f"- ASISTENCIA DE: {row.get('assistPlayerNameInitial')} (Total: {row.get('assistTotal', 0)})"
                )
        ...
        ficha_dinamica.append(
            f"- TOTAL REBOTES JUGADOR: {row.get('reboundTotal', 'N/A')}"
        )
        ...
        ficha_dinamica.append(
            f"- RECUPERADO POR: {row.get('jumpBallRecoveredName', 'N/A')}"
        )

    elif tipo in ["block", "steal"]:
        actor = (
            row.get("blockPlayerName")
            if tipo == "block"
            else row.get("stealPlayerName")
        )
        ...
        ficha_dinamica.append(
            f"- RECIBE FALTA: {row.get('foulDrawnPlayerName', 'N/A')}"
        )
        ficha_dinamica.append(
            f"- FALTAS TOTALES: {row.get('foulPersonalTotal', 'N/A')}"
        )

    elif tipo == "turnover":
        ficha_dinamica.append(f"- CAUSA: {subtipo.upper()}")
        ficha_dinamica.append(
            f"- PÉRDIDAS TOTALES JUGADOR: {row.get('turnoverTotal', 'N/A')}"
        )
