...
def get_nba_data():
    ...
    try:
        ...
        return df.with_columns([
            ...
            # Clean IDs to avoid duplicate issues
...
if not df_all.is_empty():
    ...
    # --- LEVEL 2: GAME FILTER ---
    games_meta = df_period.unique(subset=["game_id"]).sort(["game_date", "team"], descending=True)
    ...
    # Security logic: Reset if ID doesn't exist in new period
    ...
    if search:
        df_final = df_final.filter(pl.col("player_name").str.to_lowercase().str.contains(search))

    df_final = df_final.sort("pts", descending=True)

    # --- LEGEND & GLOSSARY (New Section) ---
    st.sidebar.markdown("---")
    ...
    if not df_final.is_empty():
        ...
        display_cols = ["player_name", "team", "pts", "reb", "ast", "pct_fg", "game_date"]
        ...
        event = st.dataframe(
            ...
            height=500,
            column_config={
                "player_name": st.column_config.TextColumn("Player", help="Full name of the player"),
                "team": st.column_config.TextColumn("Team", help="Player's Team Tricode"),
                "opponent": st.column_config.TextColumn("Opponent", help="Opponent Team Tricode"),
                "pts": st.column_config.NumberColumn("PTS", format="%d", help="Points Scored"),
                "reb": st.column_config.NumberColumn("REB", format="%d", help="Total Rebounds"),
                "ast": st.column_config.NumberColumn("AST", format="%d", help="Total Assists"),
                "pct_fg": st.column_config.ProgressColumn("FG%", format="%.2f", min_value=0, max_value=1, help="Field Goal Percentage"),
                "game_date": st.column_config.DateColumn("Date", format="DD/MM/YY", help="Game Date")
            }
        )
        ...
        if selected_indices:
            ...
            for i, idx in enumerate(display_indices):
                try:
                    ...
                    with cols[i]:
                        st.markdown(f"<p class='player-title-small'>{p['player_name']}</p>", unsafe_allow_html=True)
                        st.caption(f"{p['team']} vs {p['opponent']}")
                        
                        st.image(f"https://cdn.nba.com/headshots/nba/latest/1040x760/{p['player_id']}.png", width=160)
                        ...
                        fig.update_layout(
                            polar=dict(
                                ...
                                angularaxis=dict(gridcolor="#444", tickfont=dict(size=10, color="white"))
                            ),
                            height=250,
                        ...
                        st.plotly_chart(fig, use_container_width=True, key=f"radar_{p['player_id']}_{idx}")
                except Exception as e:
                    st.error(f"Error rendering player: {e}")
        else:
            st.info("👆 Select players in the table to compare their performance radars.")
    else:
